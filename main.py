...
class NodeMonitor:
    def __init__(self, config: Dict[str, Any]):
        ...
        # --- MODIFIED: Added reputation_cooldown_until to the state ---
    ...
    def _restart_container(self, container: Container, reason: str, details: str = "") -> None:
        ...
        # --- MODIFIED: Reset state and set cooldown on restart ---
        if cid in self.container_states:
            ...
            # If restart is due to reputation, set the cooldown timer
            if reason == "Reputation Failure":
                cooldown_minutes = self.config.get("reputation_restart_cooldown_minutes", 30)
                state_info["reputation_cooldown_until"] = now_utc + timedelta(minutes=cooldown_minutes)
                logging.info(f"Reputation cooldown for '{cid}' activated. Will not check reputation until {state_info['reputation_cooldown_until'].isoformat()}")
    ...
    def _check_reputation(self) -> None:
        ...
        for cid, address in node_addresses.items():
            ...
            # --- MODIFIED: Check if node is in cooldown period ---
            ...
            api_url = f"{base_url}/{address}" # Removed .lower() in previous fix
            try:
    ...
    # ... (Sisa kode lainnya seperti run(), _evaluate_all_nodes(), dll tetap sama)
    def run(self) -> None:
