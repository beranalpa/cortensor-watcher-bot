...
def load_and_validate_config(path: Path) -> Dict[str, Any]:
    """
    Loads configuration from a JSON file, validates it, and merges it
    ...
    # --- NEW: Validate Node Address Format ---
    ...
    for name, address in node_addresses.items():
        if address_pattern.match(address):
            ...
            logging.warning(f"Invalid Ethereum address format for node '{name}': {address}. This node will be skipped.")
